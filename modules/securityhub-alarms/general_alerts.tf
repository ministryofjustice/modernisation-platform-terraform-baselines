...
resource "aws_cloudwatch_log_metric_filter" "disable_alarm_actions_events" {
  name           = var.disable_alarm_actions_events_metric_filter_name
  log_group_name = var.cloudtrail_log_group_name
  pattern        = "{($.eventName = \"DisableAlarmActions\") && ${local.automation_role_filter}}"

  metric_transformation {
    name      = var.disable_alarm_actions_events_metric_name
    namespace = "LogMetrics"
    value     = 1
  }
}
...
resource "aws_cloudwatch_metric_alarm" "disable_alarm_actions_events" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = var.disable_alarm_actions_events_metric_name
  namespace           = "LogMetrics"
  period              = "300"
  statistic           = "Sum"
  threshold           = "1"
...
resource "aws_cloudwatch_log_metric_filter" "secrets_manager_events_core_accounts_mp_all" {
  ...
  name           = var.secrets_manager_events_core_accounts_mp_all_metric_filter_name
  log_group_name = var.cloudtrail_log_group_name
...
resource "aws_cloudwatch_log_metric_filter" "secrets_manager_events_core_accounts_mp_team" {
  count          = local.is_mp_workspace || local.is_mp_account ? 1 : 0
  name           = var.secrets_manager_events_core_accounts_mp_team_metric_filter_name
  log_group_name = var.cloudtrail_log_group_name
...
resource "aws_cloudwatch_metric_alarm" "secrets_manager_core_account_events_not_by_mp_team" {
  ...
  metric_query {
    ...
    metric {
      metric_name = var.secrets_manager_events_core_accounts_mp_all_metric_filter_name
      namespace   = "LogMetrics"
      period      = 300
      stat        = "Sum"
    }
  ...
  metric_query {
    ...
    metric {
      metric_name = var.secrets_manager_events_core_accounts_mp_team_metric_filter_name
      namespace   = "LogMetrics"
      period      = 300
      stat        = "Sum"
    }
...
resource "aws_cloudwatch_log_metric_filter" "s3_object_deletions_excluding_tf_lock_files" {
  count          = local.is_mp_account || (local.is_mp_workspace && !local.is_mp_sandbox) ? 1 : 0
...
resource "aws_cloudwatch_metric_alarm" "s3_object_deletions_excluding_tf_lock_files" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = var.s3_object_deletions_excluding_tf_lock_files_metric_filter_name
  namespace           = "LogMetrics"
  period              = "300"
  statistic           = "Sum"
  threshold           = "1"
...
resource "aws_cloudwatch_log_metric_filter" "ec2_termination_in_core_shared_services" {
  ...
  name           = var.ec2_termination_in_core_shared_services_metric_filter_name
  log_group_name = var.cloudtrail_log_group_name
...
resource "aws_cloudwatch_metric_alarm" "ec2_termination_in_core_shared_services" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = var.ec2_termination_in_core_shared_services_metric_filter_name
  namespace           = "LogMetrics"
  period              = "300"
  statistic           = "Sum"
  threshold           = "1"
